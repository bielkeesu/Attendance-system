...
function attendanceReducer(state, action) {
  switch (action.type) {
    ...
    //   const updatedList = state.attendances.map((att) =>
    //     att._id === updated._id ? updated : att
    //   );
    //   return { ...state, attendances: updatedList };
    // };
...
export function AttendanceProvider({ children }) {
  ...
  const fetchAttendances = async (page = 1) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/attendance?page=${page}&limit=10`);
